	...
	*/
	public function renew( $recurring = false, $status = 'active' ) {
		...
		if( $subscription->duration > 0 ) {
			...
			// add new duration to old base_date
	...
	*/
	public function can_access( $post_id = 0 ) {
		...
		if ( ! empty( $subscription_levels ) ) {
			...
			// Is there Free Member access to this content?
			$free_access = '';

			foreach ( $subscription_levels as $id ) {

				$price = rcp_get_subscription_price( $id );
				$fee = rcp_get_subscription_fee( $id );
			...
			/************************************************************
			* Only run clearance check for subscription levels if no free access.
			* This free access check is needed, otherwise regular site subscribers
			* can't see Free Member Content when using "Subscription levels"
			* instead of "Access levels".
			************************************************************/
			...
			// For free levels, simply check if user is logged in.
			} else {

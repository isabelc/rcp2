<?php
function rcp_get_email_headers() {
	global $rcp_options;
 ...
 */
function rcp_build_email( $message, $user_id, $display_name ) {
	$message = rcp_filter_email_tags( $message, $user_id, $display_name );
	// decode html
	$message = wp_specialchars_decode( $message, ENT_COMPAT );
	$message = wpautop( wp_kses_post( $message ) );
	ob_start();
	rcp_get_template_part( 'email', 'header' );
	rcp_get_template_part( 'email', 'body' );
	rcp_get_template_part( 'email', 'footer' );
	$body    = ob_get_clean();
	...
	switch ($status) :
		...
		case "active" :
			...
			if( ! isset( $rcp_options['disable_active_email'] ) ) {
				...
				$message = rcp_build_email( $message, $user_id, $user_info->display_name );
				...
				wp_mail( $user_info->user_email, $subject, $message, $headers, $attachments );
			...
			if( ! isset( $rcp_options['disable_new_user_notices'] ) ) {
				...
				$admin_message = wpautop( $admin_message );

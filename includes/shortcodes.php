...
/**
 * Displays a Welcome greeting page
 * for use on the welcome success page after a free subsription.
 */

function rcp_free_welcome_page( $atts, $content = null ) {
	...
	if( is_user_logged_in() ) {
		...
		$greet = sprintf( __( 'Welcome, %s', 'rcp' ), $current_user->display_name );
	} else {
		$greet = __( 'Welcome', 'rcp' );
	} ?>
...
add_shortcode( 'free_welcome', 'rcp_free_welcome_page' );

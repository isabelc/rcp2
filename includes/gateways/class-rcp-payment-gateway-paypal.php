...
class RCP_Payment_Gateway_PayPal extends RCP_Payment_Gateway {
	...
	public function process_signup() {
		...
		$paypal_args = array(
			...
			'bn'            => 'IsabelCastillo_ShoppingCart_RCP_US'
		);
	...
	public function process_webhooks() {
		...
		*/
		if ( $verified || isset( $_POST['verification_override'] ) || ( $this->test_mode || isset( $rcp_options['disable_ipn_verify'] ) ) )  {
			...
			$amount = empty( $posted['mc_gross'] ) ? '' : number_format( (float) $posted['mc_gross'], 2 );// isa
			$amount2 			= empty( $posted['mc_amount3'] ) ? '' : number_format( (float) $posted['mc_amount3'], 2 );// isa
			...
			$payment_data = array(
				'date'             => date( 'Y-m-d g:i:s', strtotime( $payment_date ) ),
			...
			if( $posted['txn_type'] == 'web_accept' || $posted['txn_type'] == 'subscr_payment' ) {
				...
				if ( $payment_date ) {
					...
					if( rcp_check_for_existing_payment( $posted['txn_type'], $payment_date, $subscription_key ) ) {

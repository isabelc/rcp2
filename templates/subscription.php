...
<h2><?php printf( __( 'Hello, %s', 'rcp' ), $current_user->display_name ); ?></h2>
<?php if( isset( $_GET['profile'] ) && 'cancelled' == $_GET['profile'] ) : ?>
...
<table class="rcp-table" id="rcp-account-overview">
	...
	<tbody>
		<tr>
			...
			<td><?php $sub = rcp_get_subscription(); 
					echo $sub; ?></td>
			<td><?php if ( $sub ) 
						echo rcp_get_expiration_date(); ?></td>
			<td>
				<?php
				$status = rcp_get_status( $user_ID );
				if ( 
					( // 1 of these
						rcp_is_expired( $user_ID ) ||
						'cancelled' == $status ||
						( 'active' == $status && ! rcp_is_recurring( $user_ID ) )
					) && rcp_subscription_upgrade_possible( $user_ID ) ) {
	...
	<?php if( rcp_get_user_payments() ) : ?>
		<?php foreach( rcp_get_user_payments() as $payment ) : ?>
			<tr>
				...
				<td><?php 
				// show this date in browser's tz.
				echo date_i18n( get_option( 'date_format' ), rcp_date_to_local_timezone( $payment->date ) );
				?></td>
